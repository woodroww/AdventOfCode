...
# this is the little macro that I used to make demo_input
# from the text in the problem description
# 1i'f:r'lk;DA,l1
...
def divide(r):
    ...
    #print(f"range:{r} lower:{lower} upper:{upper}")
    return lower, upper

seat_codes = real_input
#seat_codes = demo_input
seat_ids = []

for code in seat_codes:

    r = [0, 127]
    for letter in code[:-3]:
        lower_range, upper_range = divide(r)
        if letter == "F":
            r = lower_range
        elif letter == "B":
            r = upper_range
        else:
            print(f"ERROR! Que esta pasando?")
...
def non_consecutive(in_array):
    """
    Return an array of 2 len arrays where non consecutive
    [a, b] a and b are non consecutive
    """
    non_c = []
    prev_num = in_array[0]
    for i, num in enumerate(in_array[1:]):
        if prev_num + 1 != num:
            non_c.append([i, i+1])
        prev_num = num
    return non_c
...
seat_ids.sort()
non_c_idx_list = non_consecutive(seat_ids)
assert(len(non_c_idx_list) == 1)
non_c_idxs = non_c_idx_list[0]
print(f"Your seat id is: {seat_ids[non_c_idxs[0]]+1}")
