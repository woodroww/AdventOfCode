...
scores to consider.) In this example, the middle score is 288957
...
for i in range(len(nav_data)):
    ...
    if c != "":
        ...
        incomplete.append(nav_data[i])

print(f"Total error: {error_total}")


stack = []
def complete_me(incomplete_line):
    for c in incomplete_line:
        if is_open(c):
            ...
            if opening in open_set:
                open_idx = open_set.index(opening)
                if open_idx != close_set.index(c):
                    print(f"This is invalid something is very wrong")
            else:
                print(f"errorororo")
    ...
    for _ in range(len(stack)):
        opening = stack.pop()
        idx = open_set.index(opening)
        completion.append(close_set[idx])
    return completion

completions = []
for comp in incomplete:
    completions.append(complete_me(comp))
str_completions = []
for comp in completions:
   str_completions.append("".join(comp))

for i in range(len(completions)):
    print(f"{incomplete[i]} {str_completions[i]}")
...
point_list.sort()
middle_score = point_list[len(point_list) // 2]
