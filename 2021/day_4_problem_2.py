...
def main() -> int:
    ...
    while ball and (len(cards) > 0):
        ...
        for winner in winners:
            cards.remove(winner)
            long_term_winners.append(winner)
    ...
    card = long_term_winners[len(long_term_winners)-1]
    print(f"So you didn't want to win.")
    print(f"Called balls: {called_balls}")
    last_ball_number = int(called_balls[len(called_balls)-1])
    print(f"Last ball: {last_ball_number}")
    sumo = card.sum_unmarked()
    print(f"Sum of unmarked {sumo}")
    print(f"*!*!multi *jam*!*!*! winner!!&^*! {sumo*last_ball_number}")
