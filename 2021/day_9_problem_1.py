...
risk_levels = np.zeros(shape=grid.shape, dtype=np.uint8)

# find lowpoints
# if all adjacencies are greater than current
for y in range(grid.shape[1]):
    for x in range(grid.shape[0]):
        ...
        # can go left (-1)
        if x > 0:
            adjacent['left'] = grid[x-1][y]
        # can go right (+1)
        if x < grid.shape[0] - 1:
            adjacent['right'] = grid[x+1][y]
        # can go up (-1)
        if y > 0:
            adjacent['up'] = grid[x][y-1]
        # can go down
        if y < grid.shape[1] - 1:
            adjacent['down'] = grid[x][y+1]
        ...
        for key in adjacent.keys():
            if (adjacent[key] <= current_height):
                lowspot = False
        if lowspot:
            risk_levels[x][y] = 1 + current_height
