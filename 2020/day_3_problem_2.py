...
#if the square we land on after the move is a tree count it
...
for test_vals in test_slope:

    spots = []
    x_pos = 0 # left
    y_pos = 0 # top
    x_max_index = len(course[0])
    y_max_index = len(course) - 1
    tobogganing = True
    print(f"\ntest_vals: {test_vals}")
    while (tobogganing):
        ...
        if x_pos >= x_max_index:
            x_pos -= x_max_index
        if y_pos > y_max_index:
            tobogganing = False

        if tobogganing:
            spot = course[y_pos][x_pos]  
            print(spot, end=" ")
            spots.append(spot)

    test_spots.append(spots)

test_counts = []

for spot_test in test_spots:    
    tree_count = 0
    for spot in spot_test:
        if spot == "#":
            tree_count += 1
    test_counts.append(tree_count)
...
print(f"\nWell that was a journey: {total_product}")
